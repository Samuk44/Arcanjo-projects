...
async function carregarVinculos() {
  try {
    const snapshot = await get(
      ref(rtdb, `professores/${professorId}/vinculos`),
    );
    if (snapshot.exists()) {
      vinculos = Object.values(snapshot.val());
      vinculos.forEach((v) => {
...
[document.getElementById("filter-inicio"), document.getElementById("filter-fim")].forEach(function (el) {
  if (!el) return;
