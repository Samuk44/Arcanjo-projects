...
function populateChildSelector() {
  ...
  if (!selector) {
    console.warn("⚠️ #childSelector não encontrado");
    return;
  }

  if (state.children.length === 0) {
    selector.innerHTML = '<option value="">Nenhum aluno vinculado</option>';
    showToast("Nenhum aluno vinculado à sua conta", "warning");
    return;
  }

  selector.innerHTML = '<option value="">Carregando alunos...</option>';

  // Load child names from database
  const promises = state.children.map(async (childId) => {
    const childRef = ref(db, `alunos/${childId}`);
    const snapshot = await get(childRef);
    if (snapshot.exists()) {
      const childData = snapshot.val();
      return { id: childId, nome: childData.nome, turma: childData.turma };
    }
    return null;
  });
  ...
  Promise.all(promises).then((children) => {
    ...
    selector.innerHTML = '<option value="">Selecione o aluno...</option>';
    validChildren.forEach((child) => {
      const option = document.createElement("option");
      option.value = child.id;
      option.textContent = `${child.nome} - ${child.turma || "Turma não informada"}`;
      if (child.id === state.currentChild) {
        option.selected = true;
      }
      selector.appendChild(option);
    });

    console.log("✅ Alunos carregados:", validChildren.length);
  });
}
