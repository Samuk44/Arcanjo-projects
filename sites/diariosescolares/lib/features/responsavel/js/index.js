...
function renderKPIs(aluno) {
  ...
  const kpis = [
    ...
    },
    {
      label: "Comunicados",
      valor: String(comuns),
      badge: comuns > 0 ? "badge-warning" : "badge-info",
      icon: "💌",
    },
  ];
  c.innerHTML = kpis
...
function renderFeed(aluno) {
  ...
  (aluno.proximasAvaliacoes ?? [])
    .slice(0, 1)
    .forEach((a) =>
      items.push({
        icon: "📅",
        titulo: `Próxima avaliação: ${a.materia ?? ""}`,
        desc: `Data: ${a.data ?? "—"}`,
        badge: "badge-info",
        tipo: "Avaliação",
      }),
  ...
  if (!items.length) {
    c.innerHTML =
      '<div class="p-8 text-center text-sm text-muted">Nenhuma atualização recente</div>';
    return;
  }
