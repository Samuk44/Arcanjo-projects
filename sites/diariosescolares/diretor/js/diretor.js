...
const selectors = {
  ...
  logoutButtons: Array.from(document.querySelectorAll("#logoutButton, #sidebarLogout")),
};
...
const renderKpis = ({ professores, pais, alunos, turmas }) => {
  const cards = [
    createCard("Total Professores", professores, "<span class='text-xl'>👩‍🏫</span>"),
...
const buildRecentItems = (users) => {
  return users
    .map((user) => ({
      ...
      tipo: user.role === "professor" ? "Professor" : "Pai",
      data: formatDate(user.criadoEm),
      status: user.status || "ativo",
      criadoEm: typeof user.criadoEm === "number" ? user.criadoEm : Date.parse(user.criadoEm || ""),
    }))
...
const updateUserProfile = (userData) => {
  ...
  if (selectors.avatar) selectors.avatar.textContent = firstName.slice(0, 2).toUpperCase();
};

const loadDashboardData = async (diretorData) => {
  const cnpj = diretorData.instituicao?.cnpj || "";
  const usuariosRef = query(ref(db, "usuarios"), orderByChild("instituicao/cnpj"), equalTo(cnpj));
  const usuariosSnap = await get(usuariosRef);
  const usuarios = usuariosSnap.exists() ? Object.values(usuariosSnap.val()) : [];

  const professores = usuarios.filter((user) => user.role === "professor" && user.status === "ativo").length;
  const pais = usuarios.filter((user) => user.role === "pai" && user.status === "ativo").length;
  ...
  const turmas = turmasSnap.exists()
    ? Object.values(turmasSnap.val()).filter((item) => item?.ativa !== false).length
    : 0;

  renderKpis({ professores, pais, alunos, turmas });
  renderQuickActions();
  renderRecentList(buildRecentItems(usuarios.filter((user) => user.role === "professor" || user.role === "pai")));
};
...
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    if (selectors.statusMessage) {
      ...
      selectors.statusMessage.classList.add("bg-yellow-50", "border-yellow-100", "text-yellow-700");
      selectors.statusMessage.classList.remove("bg-blue-50", "border-blue-100", "text-blue-700");
    }
    return;
  }
