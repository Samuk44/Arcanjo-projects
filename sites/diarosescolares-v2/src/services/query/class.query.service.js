// src/services/school/class.query.service.js
 ...
 * - Teacher: apenas as turmas vinculadas via indexes/teacherClasses.
 */
  ...
  if (u.role === "teacher") {
    ...
    const classes = await Promise.all(
      classIds.map((classId) =>
        getClassById(u.schoolId, classId).catch(() => null),
      ),
    );

    return classes
      .filter((c) => c !== null && c.schoolId === u.schoolId)
 ...
 */
export async function getClassByIdSafe(classId) {
  const u = actor();
