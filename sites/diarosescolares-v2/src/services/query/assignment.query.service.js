...
async function getClassesFromAssignments(assignments) {
  const classRefs = [
    ...new Map(
      assignments
        .filter((item) => item.schoolId && item.classId)
        .map((item) => [`${item.schoolId}/${item.classId}`, item]),
    ).values(),
  ];

  const classes = await Promise.all(
    classRefs.map(async (item) => {
      const data = await readNode(
        `schools/${item.schoolId}/classes/${item.classId}`,
      );
      return data ? normalizeClass(item.classId, data) : null;
    }),
  );
