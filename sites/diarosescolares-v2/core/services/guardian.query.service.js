...
async function assertStudentLinked(guardianId, studentId) {
  const snap = await get(ref(db, `guardianStudents/${guardianId}/${studentId}`));
...
async function getAttendanceIdsForClass(schoolId, classId) {
  const snap = await get(ref(db, `indexes/attendanceByClass/${schoolId}/${classId}`));
  return Object.keys(snap.val() || {});
}
...
export async function getStudentAttendance(studentId) {
  ...
  for (const classId of classIds) {
    const classData = await get(ref(db, `classes/${classId}`));
