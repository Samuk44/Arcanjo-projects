...
import { createClass, getClass, updateClass, deleteClass } from './class.service.js';
...
function requireDirector(u) {
  if (u.role !== 'director') throw err('Apenas diretores podem gerenciar turmas.');
}

async function fetchOwnClass(u, classId) {
  const klass = await getClass(classId);
  if (!klass) throw err('Turma não encontrada.');
  if (klass.schoolId !== u.schoolId) throw err('Acesso negado. Esta turma não pertence à sua escola.');
  return klass;
}
...
export async function editClass(classId, data) {
  ...
  return updateClass(classId, data);
}

/**
 * Remove uma turma. Valida pertencimento à escola antes de remover.
 */
