...
async function getClassSnapshot(classId) {
...
export async function createAttendance(classId, data) {
  ...
  const classSnapshot = await getClassSnapshot(classId.trim());
  ...
  const canCreate = await permissionService.canCreateAttendance(
    { uid: teacherId.trim() },
    classId.trim(),
  );
...
export async function getAttendanceByClass(classId) {
  ...
  const ids = await fetchIdsAtPath(
    `${INDEX_BY_CLASS}/${classSnapshot.schoolId}/${classSnapshot.id}`,
  );
...
export async function getAttendanceByDate(classId, date) {
  ...
  const classSnapshot = await getClassSnapshot(classId.trim());
  if (!classSnapshot) return null;

  const dateKey = normalizeDateKey(date);
  const snap = await get(
    ref(
      db,
      `${INDEX_UNIQUE}/${classSnapshot.schoolId}/${classSnapshot.id}/${dateKey}`,
    ),
  );
