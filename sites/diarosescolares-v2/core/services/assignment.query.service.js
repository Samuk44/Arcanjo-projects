...
import { getTeacherAssignments, getClassAssignments } from './assignment.service.js';
...
export async function getAssignmentsByClass(classId) {
  ...
  if (classSnap.val().schoolId !== u.schoolId) throw err('Acesso negado. Escola incompatível.');
...
export async function getTeacherAssignmentsByDirector(teacherId) {
  const u = actor();
  if (u.role !== 'director') throw err('Apenas diretores podem consultar vínculos de outros professores.');

  const teacherSnap = await get(ref(db, `users/${teacherId}`));
  if (!teacherSnap.exists()) throw err('Professor não encontrado.');
  if (teacherSnap.val().schoolId !== u.schoolId) throw err('Este professor não pertence à sua escola.');
