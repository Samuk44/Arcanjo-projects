...
import { getSchool } from './school.service.js';
import { getUsersBySchool, getUsersByRole } from './user.service.js';
...
export async function getSchoolUsers(role) {
  const u = actor();
  if (u.role !== 'director') throw err('Apenas diretores podem listar usuários da escola.');

  if (role) {
    return getUsersByRole(u.schoolId, role);
  }

  return getUsersBySchool(u.schoolId);
}
