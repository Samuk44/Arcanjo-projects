...
function applyDateMaskToCard(el) {
  const input = el.querySelector
    ? el.querySelector(".data-nascimento-aluno")
    : el;
  if (!input) return;
  ...
  input.addEventListener("blur", function () {
    ...
    if (!v) return;
    if (v.length < 10) {
      this.setCustomValidity("Use dd/mm/aaaa");
      this.reportValidity();
      return;
    }
...
const validateStep2 = () => {
  ...
  alunosCards.forEach((card) => {
    ...
    // Cada card tem 5 .error-message: [0]=nome [1]=dataNasc [2]=turma [3]=turno [4]=periodo
    const errs = card.querySelectorAll(".error-message");
    const showCardErr = (i, msg) => {
      const el = errs[i];
      if (el) { el.textContent = msg; el.classList.add("show"); }
    };

    if (!nomeAluno || nomeAluno.length < 3) {
      showCardErr(0, "Nome do aluno deve ter pelo menos 3 caracteres.");
      isValid = false;
    }

    if (!dataNascimento) {
      showCardErr(1, "Data de nascimento é obrigatória.");
      isValid = false;
    }

    if (!turma) {
      showCardErr(2, "Turma é obrigatória.");
      isValid = false;
    }

    if (!turno) {
      showCardErr(3, "Turno é obrigatório.");
      isValid = false;
    }

    if (!periodo) {
      showCardErr(4, "Período é obrigatório.");
      isValid = false;
    }
...
const addAluno = () => {
  ...
  newCard.innerHTML = `
    <div class="flex justify-between items-center">
      ...
      <button type="button" onclick="removeAluno(this)" class="text-muted hover:text-primary transition">✕</button>
    </div>
...
const removeAluno = (btn) => {
  const card = btn.closest(".aluno-card");
  const container = document.getElementById("alunos-container");
...
// Exposto ANTES do DOMContentLoaded para que os onclick inline
// já encontrem as funções mesmo que o evento dispare antes do módulo terminar.
...
function _init() {
  ...
  document
    .querySelectorAll(".data-nascimento-aluno")
    .forEach(applyDateMaskToCard);
