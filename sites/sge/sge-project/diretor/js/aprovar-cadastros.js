...
const rbac = {
  checkPermission: async (role) => role === "diretor",
  getCurrentUser: () => ({
    uid: "dir_001",
...
async function init() {
  const isDiretor = await rbac.checkPermission(state.currentUser.role);
  if (!isDiretor) return (document.body.innerHTML = "Acesso Negado");

  FirebaseService.initList();

  // IntersectionObserver para lazy loading simulado
