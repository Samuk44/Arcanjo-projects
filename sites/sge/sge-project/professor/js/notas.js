...
const state = {
  professor: { uid: "prof_001", role: "professor", status: "ativo" },
  turmas: [],
  disciplinas: [],
  alunos: [],
...
async function init() {
  if (
    state.professor.role !== "professor" ||
    state.professor.status !== "ativo"
  ) {
...
window.addEventListener("pagehide", () => {
  console.log("Cleanup: Desconectando listeners de notas...");
});
