...
function updateUIProfile() {
  ...
  saudacao.textContent = `${saudacaoTxt}, ${state.profile.nome.split(" ")[0]}`;
  userName.textContent = state.profile.nome;
  userDisc.textContent = state.profile.disciplina || "Professor";
  userInitials.textContent = state.profile.nome
    .split(" ")
    .map((n) => n[0])
    .join("")
    .substring(0, 2)
